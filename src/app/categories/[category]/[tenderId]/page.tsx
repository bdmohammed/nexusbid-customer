...
export default function TenderDetailPage({ params }: TenderDetailPageProps) {
  ...
  const { category, tenderId: slug } = use(params);
  const { user } = useAuth();
    ...
    <main className="py-12 lg:py-20 bg-[var(--background)]">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="mb-6 text-sm text-[var(--muted)]">
          ...
          <span className="mx-2">/</span>
          <span className="text-[var(--foreground)] truncate max-w-[200px] inline-block align-bottom">
            {tender.title}
          </span>
        </div>
              ...
              {hasAccess ? (
                ...
                <Link
                  href={
                    user
                      ? "/pricing"
                      : `/login?redirect=/categories/${category}/${slug}`
                  }
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6 relative">
              ...
              {hasAccess ? (
                ...
                <div className="space-y-3 blur-xs select-none">
                  <p className="text-[var(--muted)]">
                    1. Mobilization of engineering teams and construction
                    equipment.
                  </p>
                  <p className="text-[var(--muted)]">
                    2. Preparation of detailed architectural diagrams and local
                    environmental reviews.
                  </p>
                  <p className="text-[var(--muted)]">
                    3. Execution of primary concrete structural components and
                    plumbing lines.
                  </p>
                </div>
              )}
              {!hasAccess && (
                <div className="absolute inset-0 bg-[var(--surface)]/70 backdrop-blur-[2px] rounded-2xl flex flex-col items-center justify-center p-6 text-center">
                  ...
                  <p className="text-xs text-[var(--muted)] max-w-xs mt-1">
                    Complete deliverables and requirements are restricted to
                    active plan subscribers.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6 relative">
              ...
              {hasAccess ? (
                ...
                <div className="space-y-3 blur-xs select-none font-mono">
                  <p className="text-[var(--muted)]">
                    - Minimum 5 years of active state licensing in the specific
                    domain.
                  </p>
                  <p className="text-[var(--muted)]">
                    - Underwriters Laboratory certification and bonding capacity
                    of $1,000,000.
                  </p>
                  <p className="text-[var(--muted)]">
                    - Proof of past successful government projects within the
                    region.
                  </p>
                </div>
              )}
              {!hasAccess && (
                <div className="absolute inset-0 bg-[var(--surface)]/70 backdrop-blur-[2px] rounded-2xl flex flex-col items-center justify-center p-6 text-center">
                  ...
                  <p className="text-xs text-[var(--muted)] max-w-xs mt-1">
                    Vendor compliance conditions are visible to authorized
                    subscribers only.
                  </p>
                </div>
              )}
            ...
            {!hasAccess && (
              <div className="bg-[#003EC7]/5 border border-[#003EC7]/20 rounded-2xl p-6 text-center">
                ...
                <p className="text-xs text-[var(--muted)] mt-2 leading-relaxed">
                  Gain instant access to original procurement tender documents,
                  contact email addresses, bid phone numbers, and full scope
                  documents.
                </p>
                <Link
                  href={
